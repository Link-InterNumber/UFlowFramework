...
- **State Lifecycle Hooks**: Supports `OnEnter` and `OnExit` callbacks for each state.
...
3. **State Entry/Exit Hooks**: Define lifecycle callbacks with `SetEnter` and `SetExit`.
4. **Priority Sorting**: Triggers are prioritized, ensuring higher-priority triggers execute first.
5. **State Escapes**: Define special exit logic using `SetEscape`.
6. **State Machine Control**: Supports start, stop, and restart operations.
...
### 5. **Add State Entry and Exit Hooks**

```csharp
stateMachine.SetEnter((int)MyState.Run, OnRunEnter)
...
5. **Entry/Exit Hooks**: `SetEnter` fires when entering a state, while `SetExit` fires when leaving a state or when the machine stops on that state.
6. **State Escapes**: Use `SetEscape` to define logic for stopping the state machine.
