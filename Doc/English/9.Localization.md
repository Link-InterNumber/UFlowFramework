...
To simplify the relatively complex configuration process of **Unity Localization** and integrate it with UFlow's configuration table system, we have extended `UnityEngine.UI.Text` and `TextMeshPro.TextMeshProUGUI`.
...
```  
*Note*: `SetLocalizationText()` automatically registers `onLanguageChange` events and updates text when the language changes.
...
3. **Translate and Import**  
   Translate the CSV and import it into Unity's `StringTable` (see [Unity Documentation](https://docs.unity3d.com/Packages/com.unity.localization@1.4/manual/CSV.html)).
