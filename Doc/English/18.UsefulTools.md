...
## 3. One-Click Logger Script Generation

UFlow provides a tool for automatically generating custom Logger scripts. You can open the tool window via `Tools > Editor Setting Window` and select `LoggerSetting` to open the configuration.

You can customize the configuration for the current Logger type or generate a new Logger type.

The generated logger class is formatted as follows:
