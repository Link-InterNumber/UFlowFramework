...
1. **Package AssetBundle**
   ...
   Use the `EditorBundleBuild` script provided by the framework to package the AssetBundle. The steps are as follows:
    ...
    - After the packaging is complete, a window will appear to configure which subpackages are remote subpackages, and the remote subpackages will not be loaded under the Editor.
    - This operation will package all resources into the `StreamingAssets` folder.

2. **Generate Runtime Asset Path Index File**

   The **asset path index file** is automatically generated using the Step 1 method, if the **asset path index file** is not generated using other packaging methods:

    - After completing the AssetBundle packaging, run the `Build > AssetBundle > AssetBundleMap` menu item.
...
public class ExampleBattleCase 
{
    ...
    private IEnumerator CreateEnemyPool()
    {
        ...
        // Recycle LoaderYieldInstruction instance. If not recycling, you can also directly call Dispose()
...
public interface IAssetManager
{
    ...
    // Preload asset with labels of assets
