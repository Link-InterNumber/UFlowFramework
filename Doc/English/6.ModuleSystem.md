...
Modules are automatically created when the game starts.
If you don't want modules to be created automatically, you can use the '[DonotInitModuleAutoly]' attribute, such as:

```csharp
[DonotInitModuleAutoly]
public class ManuallyCreateManager ： MonoSingleton<ManuallyCreateManager>
{
    // your code...
}

// create the module manually
ManuallyCreateManager.instance = new ManuallyCreateManager();

```

If a module has prerequisite modules, you can declare them with `[ModuleDependence(typeof(OtherModule))]`.

When `ModuleManager` creates a module, it first recursively creates its dependencies. You can also apply multiple
`ModuleDependence` attributes to the same module to declare multiple dependencies.

If any dependency fails to be created, the current module will not continue to be created, and an error log will be
written.

For example:

```csharp
[ModuleDependence(typeof(ConfigModule))]
[ModuleDependence(typeof(EventModule))]
public class GameplayModule : SingletonBase<GameplayModule>
{
    // your code...
}
```

In the example above, `GameplayModule` will ensure that `ConfigModule` and `EventModule` are created before it is
instantiated.
