...
When opening a new page, use the `replaceMode` parameter in
`UIManager.instance.PushPage<T>(object data = null, bool replaceMode = false)`. Passing in `true` will close the page at
