...
If you find writing UI scripts too tedious, you can use a UI script generation tool to create UI scripts in two simple steps.

 - Assuming your UI prefab is already created, in the `Project` window, select the prefab, and right-click to choose the `UFlow/Create UI Script` option. Select the script generation location and namespace, and the tool will generate a script template that inherits from `UIWindow`, complete with fields for the UI components.
 - Once the compilation is complete, in the `Project` window, select the prefab, right-click to choose the `UFlow/Add UI Component` option. This will automatically add UI components with the same name to the UI prefab and bind them if supported.

Since the bound components must have the same name as the prefab, it’s best to use this feature after completing your UI prefab.
