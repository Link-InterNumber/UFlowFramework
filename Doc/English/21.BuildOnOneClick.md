...
- **Configure Remote Packages**: Select `Build/AssetBundle/Bundle Manifest Configuration` from the Unity menu. In the opened window, you can choose which bundles are remote packages. When publishing, you need to upload the Bundles set as remote packages and the `remoteManifest.json` file generated under the `StreamingAssets` folder to the server (as a shortcut, you can also upload the entire `StreamingAssets` folder to the server).
...
- **Build Application Without Build Bundle**: If the AssetBundle has already been packed and you don’t want to execute the AssetBundle packing process again, you can select `Build > AssetBundle > Build Play Only` from the Unity menu to directly build the application.
