...
- **`IPoolable` Objects**: The smallest unit in the pool, which can be objects inheriting the `IPoolable` interface or `GameObject`s.
- **`LinkPool<T>`**: A container for `IPoolable` objects, responsible for **creation, storage, recycling, and destruction**.
- **`PoolGroup`**: A container for `LinkPool<T>` instances, used to organize different pools in a scene. Supports mixing `GameObject` and `IPoolable` pools.
