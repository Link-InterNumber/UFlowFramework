...
2. **Multiple Storage Formats**
    ...
    - *Note*: ScriptableObject may not support complex structures. Use JSON or binary formats for
      such cases.
...
### 🔧 7. Reading Configuration Tables in the Editor

When you need to read configuration tables in the Editor, you can refer to the following code example:

```csharp
var configInstance = new ConfigManager();
var configGroup = new ConfigGroup<EditorConfigLoader>();
configGroup.Append(configInstance.guidanceConf);
configGroup.LoadAll();
