...
在开发中，只有在`AssetsBundleManager.simulateAssetBundleInEditor = true`，框架才使用 AssetBundle 中的资源。

如果选择使用 AssetBundle 并且设置 `AssetsBundleManager.simulateAssetBundleInEditor = true` ，则需要完成以下步骤：

1. **打包 AssetBundle**
   
   使用框架提供的 `EditorBundleBuild` 脚本完成 AssetBundle 的打包。以下为打包步骤：
      - 在 Unity 编辑器菜单中，选择 `Build > AssetBundle > Build AsserBundle`。
