...
| `ChunkCustomIndexer<TKey>` | 适用于可由key直接推导chunk索引的场景，避免依赖完整键映射表。 |
...
如果你希望分块规则不再依赖“输入顺序 + 固定 chunkSize”，也可以传入一个 `key => chunkId` 方法作为分组依据：
...
- 相同 `chunkId` 的记录会被写入同一个 chunk。
- `ChunkInfo.index` 会直接保存这个 `chunkId`。
- 每个 chunk 内部仍保持输入数据原本的先后顺序。
...
### 2.2.🧭 **使用 ChunkCustomIndexer 处理可推导 key**

当 key 本身可以直接推导出所属 chunk 时，可以使用 `ChunkCustomIndexer<TKey>`。

这类场景通常有一个稳定规则，例如：

- 数值 key 按固定区间分块。
- key 中包含分区号、日期号、页码等可直接换算为 chunk 索引的信息。
- 不希望在运行时维护完整的 key 到 chunk 映射表，只需要保留 chunk 偏移表。

`ChunkDataQueryer<TKey, TData>` 提供了对应的 `Prepare` 重载：
...
如果不需要额外的 key 范围校验，也可以直接使用只传 `keyToChunkIndex` 的重载：
...
- 只接受指定范围内的 key。
- 根据 `key => (key - 1000) / 256` 直接算出目标 chunk。
- 初始化时仍会读取索引文件，但只保存 `chunkIndex -> offset` 的映射。

使用该方式时，需要保证 `keyToChunkIndex` 与写入 chunk 时的分块规则完全一致，否则会定位到错误的 chunk。

此外，当前 `ChunkDataQueryer.GetByKey(...)` 的实现依赖默认的 `ChunkIndexer<TKey>`；如果内部使用的是 `ChunkCustomIndexer<TKey>`，则更适合使用 `Get(key, ...)` 做单 key 精确读取，或自行实现批量查询逻辑。

### 2.3.🧱 **自定义构造 Chunk 数据查找工具**

如果默认的 `ChunkDataQueryer<TKey, TData>` 不满足需求，也可以自行组合底层组件，构造专用查询工具。

最常见的做法有两种：

- 使用 `ChunkDataQueryer<TKey, TData>.CreateChunkTools(...)` 快速得到默认 `ChunkIndexer<TKey>` 和 `ChunkDataMap<TKey, TData>`。
- 直接实现 `IChunkIndexer<TKey>`，再配合 `ChunkDataMap<TKey, TData>` 与 `ChunkReader` 构造自己的查询类。

例如，下面是一个最小化的自定义查询器示例：
...
8. 使用 `ChunkCustomIndexer<TKey>` 时，`keyToChunkIndex` 必须和写入时的分块规则保持一致。
9. 当前 `ChunkDataQueryer.GetByKey(...)` 依赖默认 `ChunkIndexer<TKey>`；若使用 `ChunkCustomIndexer<TKey>`，请优先使用 `Get(...)` 或自行封装批量查询。
10. 使用自定义 `chunkId` 写入时，`chunkId` 需要是非负整数；负值会被视为无效配置。
