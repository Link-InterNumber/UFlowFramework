...
- 可无缝接入其他资源管理工具（已接入 Unity Addressables、AssetBundle）。
...
1. **打包 AssetBundle**
   ...
   使用框架提供的 `EditorBundleBuild` 脚本完成 AssetBundle 的打包。以下为打包步骤：
      - 在 Unity 编辑器菜单中，选择 `Build > AssetBundle > Bundle Manifest配置`。
      - 打包完成后会出现窗口用于配置哪些分包是远程分包，在 Editor 下不会加载远程分包。
      - 该操作会将所有资源打包到 `StreamingAssets` 文件夹中。

2. **生成运行时资源路径索引文件**

    在使用步骤1方法会自动生成**资源路径索引文件**，如果使用其他打包方法没有生成 **资源路径索引文件**：
    - 在完成 AssetBundle 打包后，运行 `Build > AssetBundle > AssetBundleMap` 菜单项。
...
public class ExampleBattleCase 
{
    ...
    private IEnumertor CreateEnemyPool()
    {
        ...
        // 回收LoaderYieldInstruction资源，不回收的情况下也可直接调用 Dispose()
...
以下为示例：
 ...
 public class ExampleAssetManager : IAssetManager
 {
    ...
    public void Unprepare(PrepareHandler)
    {
