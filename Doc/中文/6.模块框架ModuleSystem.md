...
模块会在游戏启动时自动创建。
如果不希望模块自动创建，可以使用 `[DonotInitModuleAutoly]` 标记，如：
...
如果模块存在前置依赖，可以使用 `[ModuleDependence(typeof(OtherModule))]` 声明依赖关系。

`ModuleManager` 在创建模块时，会先递归创建依赖模块；同一个模块也可以重复标注多个 `ModuleDependence` 来声明多个依赖。

如果某个依赖模块创建失败，那么当前模块也不会继续创建，并输出对应错误日志。
...
上面的 `GameplayModule` 在实例化前，会先确保 `ConfigModule` 和 `EventModule` 已经创建完成。
