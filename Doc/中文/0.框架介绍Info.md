...
- 基于EEPlus开发的配置表导入工具，支持excle和csv格式的配置表格，将表格数据生成为按块索引（index-chunk）的二进制的数据。
- 提供便捷的工具类和窗口，可以一键式生成配置表类、配置表资源，并支持将生成配置表流程接入到自定义构建流程中。
- 二进制的配置表资源提供加密/解密。

### 4.📢 **事件系统**

- 内置事件管理器，支持全局/基于作用域的事件订阅和广播。
...
└─── UFlowFramework/                # 框架核心代码
    ...
    └── Toolkit/                    # 一些使用的测试工具、UI组件、功能代码
        ...
        ├── Debug/                  # 调试与测试基类
        ├── LinkState/              # 有限状态机
        ├── ListUpdaters/           # UI列表更新器
        ├── MathToolkit/            # 数学工具
        ├── ObjectInteractor/       # 物体交互辅助工具
        ├── PostEffect/             # 后处理与表现辅助工具
        ├── Reflection/             # 反射工具类
        ├── Serialization/          # 序列化与加密相关工具
        ├── Shake/                  # 震动工具类
        └── UIToolKit/              # UI工具
```