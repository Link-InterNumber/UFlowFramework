...
- **配置远程包**：选择Unity菜单 `Build/AssetBundle/Bundle Manifest配置`（也可以在打包/构建时选择是否重新配置），可以在打开的窗口中选择那些分包是远程分包，在发布时需要将设置为远程分包的Bundle和生成在 `StreamAssets` 文件夹下的 `remoteManifest.json` 文件发布到服务器上（偷懒的话也可以将整个 `StreamAssets` 文件夹放到服务器）。
...
- **构建应用**：在Unity菜单中选择 `Build > AssetBundle > Build Play`，可以一键完成AssetBundle打包，并开始构建应用。
- **跳过打包构建应用**：如果已经打包好AssetBundle，不想再执行AssetBundle打包流程，可以在Unity菜单中选择 `Build > AssetBundle > Build Play Only`，直接构建应用。
