...
- **配置远程包**：选择Unity菜单 `Build/AssetBundle/Bundle Manifest配置`，可以在打开的窗口中选择那些分包是远程分包，在发布时需要将设置为远程分包的Bundle和生成在 `StreamAssets` 文件夹下的 `remoteManifest.json` 文件发布到服务器上（偷懒的话也可以将整个 `StreamAssets` 文件夹放到服务器）。
