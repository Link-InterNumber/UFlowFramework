...
`LocalizationManager` 中使用 `ILocalizationProvider` 对象获取本地化配置。开发者可以实现 `ILocalizationProvider` 接口来接入自定义的本地化方案。

例如，开发者使用excel表格配置本地化数据，以下为伪代码示例：
```csharp
...
之后，在 `LocalizationManager` 初始化时，将实例化的 `ILocalizationProvider` 对象传入：

```csharp
yield return LocalizationManager.instance.Init(new ExcelLocalizationProvider(), null);
```

之后就可以使用上面的方法调用 `LocalizationManager`。
---
