...
值得注意的是，`RuntimeDataManager` 存在一个 *partial* 方法 `InitRuntimeData`，用于在 `EventManager.instance.onStartGame` 被触发时，可以对数据进行生成操作。
使用方式就是：
   - 在你项目代码中创建一个 "RuntimeDataManager_InitRuntimeData.cs" 脚本；
   - 在新建的脚本内实现 `InitRuntimeData` 方法。
```csharp
