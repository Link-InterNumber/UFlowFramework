...
- 配置以下参数：
  - `Long Press Start Time`：设置长按开始的时间阈值（单位：秒）。
  - `Long Press Trigger Time`：设置长按触发的时间阈值（单位：秒）。
  - `Long Press Interval Time`（可选）：设置长按重复触发的时间间隔（单位：秒）。
...
```
### 注意事项
- 如果启用了长按功能，短按事件（如 `OnClick`）将在长按时间阈值内被屏蔽。
...
`uGUI-Hypertext` 是一个扩展自 Unity `Text` 的自定义组件，支持在文本中嵌入超链接并响应点击事件。它适用于需要在 UI 文本中实现交互功能的场景。

详细使用方法和示例代码请参考 [uGUI-Hypertext](https://github.com/setchi/uGUI-Hypertext)

# 网络通信工具 Network Communication Tools

`UFlowFramework` 提供了一套简单的网络通信工具，支持客户端与服务器之间的高效通信。以下是主要功能和使用方法：
