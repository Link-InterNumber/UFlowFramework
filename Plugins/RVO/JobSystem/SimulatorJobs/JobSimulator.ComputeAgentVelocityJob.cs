...
namespace RVO.JobSystem
{
    public sealed partial class JobSimulator
    {
        ...
        private struct ComputeAgentVelocityJob : IJobParallelFor
        {
            ...
            [NativeDisableParallelForRestriction]
            public NativeArray<int> ObstacleOrcaLineCounts;
            [NativeDisableParallelForRestriction]
            ...
            public void Execute(int index)
            {
                ...
                var invTimeHorizonObst = 1.0f / math.max(agent.timeHorizonObst, RvoEpsilon);
                ...
                for (var slot = 0; slot < obstacleNeighborCount; slot++)
                {
                    ...
                    for (var existing = 0; existing < lineCount; existing++)
                    {
                        ...
                        var left = MathUtil.Det(invTimeHorizonObst * relativePosition1 - line.point, line.direction) - invTimeHorizonObst * agent.radius;
                        var right = MathUtil.Det(invTimeHorizonObst * relativePosition2 - line.point, line.direction) - invTimeHorizonObst * agent.radius;
                        if (left >= -RvoEpsilon && right >= -RvoEpsilon)
                        {
                            alreadyCovered = true;
                            break;
                        }
                    ...
                    var obstacleVectorAbsSq = math.max(MathUtil.AbsSq(obstacleVector), RvoEpsilon);
                    ...
                    if (s < 0.0f && distSq1 <= radiusSq)
                    {
                        if (obstacle1.convex == 1)
                        {
                            ...
                            constraint.direction = MathUtil.NormalizeSafe(new float3(-relativePosition1.y, relativePosition1.x, 0.0f));
                    ...
                    if (s > 1.0f && distSq2 <= radiusSq)
                    {
                        ...
                        {
                            constraint.point = float3.zero;
                            constraint.direction = MathUtil.NormalizeSafe(new float3(-relativePosition2.y, relativePosition2.x, 0.0f));
                            if (lineCount < MaxOrcaLineCapacity)
                            {
                                OrcaLines[lineStart + lineCount] = constraint;
                                lineCount++;
                    ...
                    if (s < 0.0f && distSqLine <= radiusSq)
                    {
                        ...
                        leftLegDirection = new float3(
                            ...
                            0.0f) / math.max(distSq1, RvoEpsilon);
                    ...
                    else if (s > 1.0f && distSqLine <= radiusSq)
                    {
                        ...
                        leftLegDirection = new float3(
                            ...
                            0.0f) / math.max(distSq2, RvoEpsilon);
                        rightLegDirection = new float3(
                            relativePosition2.x * leg2 + relativePosition2.y * agent.radius,
                            -relativePosition2.x * agent.radius + relativePosition2.y * leg2,
                            0.0f) / math.max(distSq2, RvoEpsilon);
                    ...
                    var cutOffAbsSq = math.max(MathUtil.AbsSq(cutOffVector), RvoEpsilon);
                    ...
                    if ((t < 0.0f && tLeft < 0.0f) || (leftObstacle.id == rightObstacle.id && tLeft < 0.0f && tRight < 0.0f))
                    {
                    ...
                    {
                        constraint.direction = -obstacle1.direction;
                        constraint.point = leftCutOff + agent.radius * invTimeHorizonObst * new float3(-constraint.direction.y, constraint.direction.x, 0.0f);
                        if (lineCount < MaxOrcaLineCapacity)
                        {
                            OrcaLines[lineStart + lineCount] = constraint;
                            lineCount++;
                    ...
                    constraint.point = rightCutOff + agent.radius * invTimeHorizonObst * new float3(-constraint.direction.y, constraint.direction.x, 0.0f);
                    if (lineCount < MaxOrcaLineCapacity)
                    {
                        OrcaLines[lineStart + lineCount] = constraint;
                        lineCount++;
                ...
                var lineFail = LinearProgram2(OrcaLines, lineStart, lineCount, agent.maxSpeed, agent.prefVelocity, false, ref newVelocity);
