...
namespace PowerCellStudio
{
    ...
    public sealed class JobRunner : IDisposable
    {
        ...
        public JobRunner(CoroutineRunner coroutineRunner)
        {
           ...
           where T : struct
        {
            ...
            _coroutineRunner.RunCoroutine(AsyncRunParallelJobCorount(job, resultArray, length, onComplete, batchCount));
#else
            JobHandle handle = job.ScheduleParallel(length, batchCount, default);
            _asyncJobs.Add(new JobInfo
            {
        ...
        public void Tick()
        {
            if (_asyncJobs.Count == 0) return;
            // WebGL不支持JobSystem，无需处理异步Job
            // 检查异步Job是否完成
            for (int i = 0; i < _asyncJobs.Count; )
            {
