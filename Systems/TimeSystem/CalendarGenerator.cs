...
namespace PowerCellStudio
{
    public class CalendarGenerator
    {
        ...
        private static string[] _solarTerms =
        {
            "小寒", "大寒", "立春", "雨水", "惊蛰", "春分", "清明", "谷雨", "立夏", "小满", "芒种", "夏至", "小暑", "大暑", "立秋", "处暑", "白露", "秋分",
            "寒露", "霜降", "立冬", "小雪", "大雪", "冬至"
        };

        private static int[] _solarTermsData =
        {
        ...
        public static string GetLunarDate(DateTime date, out string lunarFestival)
        {
            try
            {
                ...
                else
                {
                    ...
                    for (int i = 1; i <= 24; i++)
                    {
                        num = 525948.76 * (y - 1900) + _solarTermsData[i - 1];
                        dtNew = baseDate.AddMinutes(num);
                        if (dtNew.DayOfYear == date.DayOfYear)
                        {
                            tempStr = _solarTerms[i - 1];
                            break;
                        }
                    }
                    lunarFestival = tempStr;
                }

                return
                    $"{(isLeapMonth ? "闰" : "")}{lunarMonth.IntToChineseMonth(isTran)} {lunarDay.IntToChineseDay(isTran)}";
            }
            catch
            {
                lunarFestival = string.Empty;
