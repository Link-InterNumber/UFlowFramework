...
namespace PowerCellStudio.Editor
{
    ...
    {
        internal static List<BundleCompareItem> Compare(
            string directory,
            string manifestName,
            ...
            IReadOnlyList<BundleBuildBaselineInfo> baseline)
        {
            ...
            var currentNames = queryer.GetAllBundleData().Keys;
            ...
            var builtData = new Dictionary<string, BuiltBundleData>(StringComparer.OrdinalIgnoreCase);

            foreach (var bundleName in allNames.OrderBy(name => name, StringComparer.OrdinalIgnoreCase))
            {
                var builtPath = BundleReferenceCompareUtility.FindBundlePath(directory, bundleName, manifestName);
                if (!builtData.TryGetValue(bundleName, out var builtAssets))
                {
                    builtAssets = BundleReferenceCompareUtility.ReadBuiltAssets(builtPath);
                    builtData[bundleName] = builtAssets;
                }

                BundleReferenceCompareUtility.CollectDependencyData(
                    bundleName, directory, manifestName, BundleReferenceManifest.manifest, builtData);

                var currentAssets = BundleReferenceCompareUtility.GetCurrentAssets(queryer, bundleName);
                var isCurrentBundle = queryer.GetAllBundleData().ContainsKey(bundleName);
                var status = GetStatus(builtAssets, currentAssets, isCurrentBundle);
                var item = new BundleCompareItem(
                    bundleName,
                    status,
                    builtAssets.size,
                    builtAssets.assetNames,
                    currentAssets,
                    builtAssets.types,
                    builtAssets.dependentBundles,
                    builtAssets.loadCost);

                ApplyBaseline(item, builtAssets, baselineData);
                items.Add(item);
            }
