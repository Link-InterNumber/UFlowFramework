...
#forend
            ...
            ConfigLogger.Log("Config Asset Created => [{{ConfName}}]");
            return md5;
        }
        #endif
    }
    ...
    public class WriteConfCreatorHandler
    {
            ...
            bool isEnumKey, bool isMultiKey)
        {
            ...
            csWriter.StartWriteMethod(CsWriter.MethodSign.Public, CsWriter.MethodSign.Static, "string", "CreatAsset", "string oldMd5")
                .WriteLine("var excelPath = EditorSaveUtils.GetEditorPref(ConfigSettingLogic.SaveKey.excelPath, \"\");")
            ...
            csWriter.StartWriteIf("!File.Exists(path)")
                .WriteLine("ConfigLogger.LogError(\"Cannot find file \" + path);")
            ...
            csWriter.WriteVar("fileName", $"\"{confName}\"")
                .WriteLine("reader.StartReadLine(3);")
                .WriteLine("var stringDataList = reader.GetNextLine();")
                .WriteLine("var row = 4;")
                .WriteLine("while (stringDataList != null)");
            csWriter.StartWriteBody();

            csWriter.WriteVar("firstCell", $"stringDataList[{miniColumn - 1}]")
            ...
            csWriter.WriteLine($"ConfigLogger.Log(\"Config Asset Created => [{confName}]\");")
                .WriteLine("return md5;")
                .EndWriteBody()
                .EndWriteMethod();
        }
        ...
        private static void WriteParseStatements(CsWriter csWriter, ConfigTypeInfo[] configTypeInfoList)
        {
            for (var i = 0; i < configTypeInfoList.Length; i++)
            {
                ...
                if (configTypeInfo.IsList)
                {
                    ...
                    {
                        csWriter.StartWriteIf($"stringDataList[{column - 1}] != null")
                            .WriteLine($"{fieldName}.Add({configTypeInfo.refTypeName}.Parse(stringDataList[{column-1}], fileName, row, {column}));")
                            .EndWriteIf();
                    }
                }
                else
                {
                    ...
                    csWriter.WriteLine(
                        $"var {fieldName} = {configTypeInfo.refTypeName}.Parse(stringDataList[{column-1}], fileName, row, {column});");
                }
            }
        }
