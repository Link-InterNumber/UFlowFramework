...
{
    public class AnimationMixerPro: LinkPlayableBehaviour
    {
        ...
        public void Init(PlayableGraph graph, ScriptPlayable<AnimationMixerPro> playable)
        {
            _playableGraph = graph;
            _playable = playable;
            _mixer = AnimationMixerPlayable.Create(_playableGraph);
            _mixer.SetOutputCount(1);
            _targetWeights = new List<float>();
            _originWeights = new List<float>();
        }

        public static ScriptPlayable<AnimationMixerPro> Create(PlayableGraph graph)
        {
            var playable = ScriptPlayable<AnimationMixerPro>.Create(graph);
        ...
        private void StartFade(float[] targetWeights, float fadeTime, Action<Playable> onCompleted)
        {
            ...
            for (int i = 0; i < inputCount; i++)
            {
                ...
                if(originWeight + targetWeight == 0) _mixer.GetInput(i).Pause();
                else _mixer.GetInput(i).Play();
            }
            _fadeTime = fadeTime;
            _fadeTimeout = 0f;
            _fading = true;
        ...
        public void AddInputDirectly(Playable input, int inputOutputPort, float weight)
        {
            weight = Mathf.Clamp01(weight);
            _mixer.AddInput(input, inputOutputPort, weight);
        }

        public void AddInput(Playable input, int inputOutputPort, float weight, float fadeTime = 0f, Action<Playable> onCompleted = null)
        {
            if (weight <= 0f)
            {
        ...
        public void SetWeight(int inputPortIndex, float weight, float fadeTime = 0f, Action<Playable> onCompleted = null)
        {
            weight = Mathf.Clamp01(weight);
            List<float> weights = new List<float>();
            var inputCount = _mixer.GetInputCount();
