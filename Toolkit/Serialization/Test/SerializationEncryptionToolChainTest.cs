...
namespace PowerCellStudio
{
    public class SerializationEncryptionToolChainTest : RunTestMono
    {
        ...
        {
            RunTest("SerializeUtils Async Binary RoundTrip", async () =>
            {
                TestPayload source = CreatePayload();

                byte[] bytes = await SerializeUtils.SerializeToBinaryAsync(source);
                TestPayload clone = await SerializeUtils.DeserializeFromBinaryAsync<TestPayload>(bytes);
