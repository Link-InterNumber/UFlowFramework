...
namespace PowerCellStudio
{
    public class BinarySerializerTest : RunTestMono
    {
        ...
        private class CustomEncodedValueSelector : IBinarySerializerTypeSelector
        {
            public Type TargetType => typeof(CustomEncodedValue);

            public void Write(BinaryWriter writer, object value, Encoding encoding)
            {
                CustomEncodedValue payload = (CustomEncodedValue)value;
                writer.Write(payload.Number);

                byte[] bytes = encoding.GetBytes(payload.Text ?? string.Empty);
