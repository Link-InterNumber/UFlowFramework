    ...
    public class BinarySerializer
    {
        ...
        public static byte[] Serialize<T>(T obj)
        {
            ...
            using (var writer = new BinaryWriter(ms, Encoding))
            {
                ...
                {
                    // Fallback if TryGetBuffer fails (should be rare)
                    var uncompressedData = ms.ToArray();
                    return LZ4Pickler.Pickle(uncompressedData);
                }
            }
        }
        ...
        /// 将对象序列化为字节数组。
        /// </summary>
        public static void SerializeAsSpan<T>(T obj, IBufferWriter<byte> destination)
        {
            var type = typeof(T);
            if (!IsSupportedType(type))
                throw new NotSupportedException($"[BinarySerializer] 不支持的类型，无法序列化。类型: {obj?.GetType()}");
            ...
            using (var writer = new BinaryWriter(ms, Encoding))
            {
                ...
                if (ms.TryGetBuffer(out var buffer))
                {
                    LZ4Pickler.Pickle(buffer, destination);
                }
                else
                {
                    // Fallback if TryGetBuffer fails (should be rare)
                    var uncompressedData = ms.ToArray();
                    LZ4Pickler.Pickle(uncompressedData, destination);
                }
            }
        }
        ...
        /// 从字节数组反序列化为指定类型的实例。
        /// </summary>
        public static T Deserialize<T>(byte[] data, int offset = 0, int count = -1)
        {
            if (!IsSupportedType(typeof(T)))
                throw new NotSupportedException($"[BinarySerializer] 不支持的类型，无法反序列化。类型: {typeof(T)}");
            var actualCount = count == -1 ? data.Length - offset : count;
            if (offset < 0 || actualCount < 0 || offset + actualCount > data.Length)
                throw new ArgumentOutOfRangeException(nameof(offset), "Offset and count must specify a valid range in the byte array.");
            var unpickledData = LZ4Pickler.Unpickle(data, offset, actualCount);
            using (var ms = new MemoryStream(unpickledData))
            using (var reader = new BinaryReader(ms, Encoding))
            {
                return BinaryFormatterResolver.GetFormatter<T>().Read(reader, Encoding);
